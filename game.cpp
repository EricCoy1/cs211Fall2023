...
int main()
{
  ...
  //pthread_create(&tAddQues, NULL, &makeQuestion, NULL);

  //thread for the user to answer questions removed from the front of the queue
  //pthread_create(&tAns, NULL, &????????, NULL);
...
void *addQuestion(void* data)
{
    ...
    clock_t waitTime = CLOCKS_PER_SEC * ((level * 5) + 1) % 5/4.0;
